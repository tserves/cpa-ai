...
import { CheckCircle2, AlertTriangle, RefreshCw, Clock, FileText, FileSpreadsheet, Image } from 'lucide-react';
...
export default function FileProgressPanel({ fileProgress }) {
  if (!fileProgress || !fileProgress.length) return null;
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5 max-h-48 overflow-y-auto">
        {fileProgress.map((file, i) => {
          ...
          const fileIcon = FILE_TYPE_ICONS[file.file_type] || FILE_TYPE_ICONS.pdf;
          return (
            <div key={i} className="flex items-center gap-3 rounded-lg border bg-card px-3 py-2">
              {fileIcon}
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium truncate">{file.name}</p>
                {file.tx_count != null && file.status === 'done' && (
                  <p className="text-[10px] text-muted-foreground">{file.tx_count} transactions extracted</p>
                )}
                {file.error && <p className="text-[10px] text-red-600 truncate">{file.error}</p>}
              </div>
