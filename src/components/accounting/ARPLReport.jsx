...
function PLSection({ title, lines = [], total, colorClass, defaultOpen = false }) {
...
function SubtotalRow({ label, value, colorClass, large = false, border = false }) {
  return (
    <div className={`flex justify-between items-center px-4 py-3 rounded-xl ${border ? 'border-2 border-primary/20 bg-primary/5' : 'bg-muted/40'}`}>
      <span className={`font-bold ${large ? 'text-base' : 'text-sm'}`}>{label}</span>
      <div className="flex items-center gap-2">
        {value < 0 ? <TrendingDown className="w-4 h-4 text-red-500" /> : <TrendingUp className="w-4 h-4 text-green-500" />}
        <span className={`font-bold font-mono ${large ? 'text-xl' : 'text-base'} ${colorClass}`}>${Math.abs(value).toFixed(2)}</span>
...
export default function ARPLReport({ report }) {
  ...
  const getQ = (key) => {
    const filtered = months.filter(m => {
      ...
      if (view === 'q1') return mo <= 3;
      if (view === 'q2') return mo >= 4 && mo <= 6;
      if (view === 'q3') return mo >= 7 && mo <= 9;
      if (view === 'q4') return mo >= 10;
      return true;
    });
    return filtered.reduce((s, m) => s + (monthlyData[m]?.[key] || 0), 0);
  };

  const rev = view === 'all' ? report.revenue : getQ('revenue');
  const cogs = view === 'all' ? report.cogs : getQ('cogs');
  const opex = view === 'all' ? report.operating_expenses : getQ('opex');
  const oi = view === 'all' ? report.other_income : getQ('other_income');
  const oe = view === 'all' ? report.other_expenses : getQ('other_expenses');
  const gp = rev - cogs;
  const noi = gp - opex;
  const np = noi + oi - oe;
  ...
  return (
    <div className="space-y-4">
      ...
      <PLSection title="Revenue" lines={report.revenue_lines || []} total={rev} colorClass="text-green-600" defaultOpen />
      ...
      <SubtotalRow label="Net Profit / Loss" value={np} colorClass={np >= 0 ? 'text-green-700' : 'text-red-700'} large border />
          ...
          <div className="rounded-xl border overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-muted/40 border-b">
                <tr>{['Month','Revenue','COGS','Op. Expenses','Net P/L'].map(h => <th key={h} className="px-3 py-2 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{h}</th>)}</tr>
              </thead>
              <tbody>
                {months.map(m => {
