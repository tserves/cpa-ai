...
export default function ARMonthlySummary({ report }) {
  if (!report?.months?.length) return (
    <div className="text-center py-10 text-muted-foreground text-sm">No monthly summary data available.</div>
  );
  ...
  const chartData = report.months.map(m => ({
    name: m.month,
    Credits: parseFloat((m.total_credits || 0).toFixed(2)),
    Debits: parseFloat((m.total_debits || 0).toFixed(2)),
  }));

  const totalCredits = report.months.reduce((s, m) => s + (m.total_credits || 0), 0);
  const totalDebits = report.months.reduce((s, m) => s + (m.total_debits || 0), 0);
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-3">
        <Card className="p-3 text-center">
          ...
          <p className="text-base font-bold font-mono text-green-600">{fmt(totalCredits)}</p>
        </Card>
        <Card className="p-3 text-center">
          <p className="text-[10px] text-muted-foreground">Total Debits</p>
          <p className="text-base font-bold font-mono text-red-600">{fmt(totalDebits)}</p>
        </Card>
        <Card className="p-3 text-center">
      ...
      <div className="overflow-x-auto rounded-xl border">
        <table className="w-full text-left">
          <thead className="bg-muted/50">
            <tr>
              {['Month', 'Transactions', 'Total Credits', 'Total Debits', 'Net', 'Review Items'].map(h => (
          ...
          <tbody>
              ...
              return (
                <tr key={i} className="border-t hover:bg-muted/10">
                  ...
                  <td className="px-3 py-2 text-xs">
                    {m.review_count > 0 ? <span className="text-amber-600 font-medium">⚠ {m.review_count}</span> : <span className="text-muted-foreground">—</span>}
                  </td>
                </tr>
              );
            })}
            <tr className="border-t bg-muted/30 font-bold">
              ...
              <td className="px-3 py-2 text-xs font-mono text-green-600">{fmt(totalCredits)}</td>
              <td className="px-3 py-2 text-xs font-mono text-red-600">{fmt(totalDebits)}</td>
              <td className={`px-3 py-2 text-xs font-mono ${(totalCredits - totalDebits) >= 0 ? 'text-green-600' : 'text-red-600'}`}>{fmt(totalCredits - totalDebits)}</td>
