...
const CAT_LABELS = {
  assets: 'Assets', revenue: 'Revenue', cogs: 'Cost of Goods Sold',
  operating_expenses: 'Operating Expenses', liabilities: 'Liabilities',
  equity: 'Equity', other_income: 'Other Income', other_expenses: 'Other Expenses', unclassified: 'Unclassified',
};
const CAT_COLORS = {
  assets:'bg-blue-100 text-blue-700', revenue:'bg-green-100 text-green-700',
  cogs:'bg-orange-100 text-orange-700', operating_expenses:'bg-red-100 text-red-700',
  liabilities:'bg-purple-100 text-purple-700', equity:'bg-indigo-100 text-indigo-700',
  other_income:'bg-teal-100 text-teal-700', other_expenses:'bg-rose-100 text-rose-700',
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {acct.account_code && <p className="text-xs text-muted-foreground">{acct.account_code}</p>}
          </div>
        </div>
        <div className="flex items-center gap-6 flex-shrink-0 text-right">
          <div className="hidden sm:block">
            <p className="text-[10px] text-muted-foreground">Debits</p>
            <p className="text-xs font-mono font-semibold text-red-600">${acct.debit_total.toFixed(2)}</p>
          </div>
          <div className="hidden sm:block">
            <p className="text-[10px] text-muted-foreground">Credits</p>
            <p className="text-xs font-mono font-semibold text-green-600">${acct.credit_total.toFixed(2)}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground">Net</p>
            <p className={`text-xs font-mono font-bold ${net < 0 ? 'text-green-600' : 'text-red-600'}`}>${Math.abs(net).toFixed(2)}</p>
          </div>
          <span className="text-xs text-muted-foreground">{acct.transactions.length} txns</span>
        </div>
      </button>
        ...
        <div className="border-t overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-muted/30">
              <tr>
                {['Date', 'Description', 'Reference', 'Debit', 'Credit', 'Balance', 'Source'].map(h => (
                  <th key={h} className="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {acct.transactions.map((tx, i) => (
                <tr key={i} className="border-t hover:bg-muted/20">
                  ...
                  <td className="px-3 py-2 text-[10px] text-muted-foreground truncate max-w-[120px]">{tx.source_file || '—'}</td>
                </tr>
              ))}
              <tr className="border-t bg-muted/30 font-semibold">
                <td colSpan={3} className="px-3 py-2 text-xs">Closing Balance</td>
...
export default function GLReportView({ report }) {
  ...
  const filtered = accounts.filter(a =>
    !search || a.account_name?.toLowerCase().includes(search.toLowerCase()) || a.category?.includes(search.toLowerCase())
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="p-3 text-center">
          ...
          <p className="text-lg font-bold font-mono text-red-600">${(report.total_debits || 0).toFixed(2)}</p>
        </Card>
        <Card className="p-3 text-center">
          <p className="text-xs text-muted-foreground">Total Credits</p>
          <p className="text-lg font-bold font-mono text-green-600">${(report.total_credits || 0).toFixed(2)}</p>
        </Card>
        <Card className="p-3 text-center">
      ...
      <div className="relative">
        ...
        <Input className="pl-8 h-8 text-xs" placeholder="Filter accounts…" value={search} onChange={e => setSearch(e.target.value)} />
      </div>
