...
export default function BKFileProgress({ fileProgress, label = 'Extracting' }) {
  if (!fileProgress?.length) return null;
  const done = fileProgress.filter(f => f.status === 'done').length;
  const failed = fileProgress.filter(f => f.status === 'failed').length;
  const pct = Math.round((done / fileProgress.length) * 100);

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between text-xs">
        <span className="font-semibold">{label} {fileProgress.length} file{fileProgress.length !== 1 ? 's' : ''}…</span>
        <span className="text-muted-foreground">{done}/{fileProgress.length} complete{failed > 0 ? ` · ${failed} failed` : ''}</span>
      </div>
        ...
        {fileProgress.map((f, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2.5">
              ...
              {f.error && <span className="text-[10px] text-red-600 truncate max-w-[120px]" title={f.error}>Error</span>}
            </div>
          );
        })}
      </div>
